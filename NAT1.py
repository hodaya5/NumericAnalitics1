...
def DominantDiagonalFix(matrix):
   ...
   # Find the largest organ in a row
    ...
    for i in range(len(matrix)):
        ...
        if i not in dom:
            print("There is not dominant diagonal.")
            return matrix
...
def matrixAddition(matrixA, matrixB):
    """
       :param matrixA, matrixB: Matrix nxn, Matrix nxn
...
def JacobiMethod(matrix, vector, epsilon, previous, counter):
    ...
    # Result vector insertion
    print("Iteration "+str(counter)+" " +str(NextGuess))
...
print("matrixA = [[4,2, 0],[2, 10, 4], [0, 4, 5]]\nb = [2,6,5]\n")
input=int(input("Which method do u want? \n\t1.Jacobi \n\t2.GaussSeidel\n"))
while (flag):
    if input == 1 :
        flag = False
        # Jacobi
        ...
        if CheckDominantDiagonal(matrixA):
            ...
            print("There is not a dominant diagonal.")
            print("Change lines.")
            dominantFix=DominantDiagonalFix(matrixA)
            PrintMatrix(dominantFix)
            if dominantFix != matrixA:
